...
\def\WorkTitle{Developing a driver for a film scanner by means of USB sniffing and reverse engineering}
\def\WorkAuthor{Hugo Platzer}
\def\WorkAuthorMail{hplatzer@cosy.sbg.ac.at}
...
A USB cable has four wires: one as ground, VBUS for a 5 V power supply and two for data transmission (\cref{fig:usbcable}).
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{2}{\input{images/tikz/usb_cable.tex}}
  \label{fig:usbcable}
\end{figure}
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{0.35}{\input{images/tikz/usb_nrzi.tex}}
\end{figure}
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{1}{\input{images/tikz/usb_tokenpacket.tex}}
  \label{fig:tokenpacket}
\end{figure}

Token packets (\cref{fig:tokenpacket}) are used at the start of so-called transactions to specify the target
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{1}{\input{images/tikz/usb_datapacket.tex}}
\end{figure}
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{1}{\input{images/tikz/usb_handshakepacket.tex}}
\end{figure}
...
Transfers (\cref{fig:usbtransfer}) are the abstraction level of data exchange as seen at the software
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{1}{\input{images/tikz/usb_composition.tex}}
...
This is useful for device developers, driver developers, keyloggers and other
applications.
...
\item Present all packets transmitted, including unsuccessful transactions
\end{itemize}
...
This becomes evident when looking at the details of such a bulk transfer in Wireshark (\cref{fig:bulkxfer_wireshark}),
where the "Data length" (captured payload bytes) is smaller than the URB length (total payload bytes).
When reconstructing the image from this recording, tearing artifacts occur (\cref{fig:bulkxfer_tearing})
because in the pixel stream missing bytes cause parts of some rows to be missing, leading to
misalignment in subsequent rows.
Others have faced this problem, unsure where it comes from and what to do (\cite{usbmon_others}).

\begin{figure}[H]
  \caption{A large bulk transfer in Wireshark}
  \centering
  \includegraphics[width=0.8\textwidth]{images/jpg/usbmon_wireshark.jpg}
  \label{fig:bulkxfer_wireshark}
\end{figure}

\begin{figure}[H]
  \caption{Image reconstructed from payloads}
  \centering
  \includegraphics[width=0.8\textwidth]{images/jpg/usbmon_chopped.jpg}
  \label{fig:bulkxfer_tearing}
\end{figure}
...
\subsubsection{Sniffing procedure when using Wireshark}
...
payload length and other criteria.
...
When the device is connected, a process like in \cref{devinit} starts.
...
\begin{figure}[H]
  ...
  \label{fig:usbkeyboard_reports}
\end{figure}

\cref{fig:usbkeyboard_reports} shows the HID reports send by the analyzed
keyboard while pressing and releasing the 'a' and 'b' keys.
In \cite[p.53]{usbusage} the byte values 04 and 05 stand for the 'a' and 'b' keys respectively.
...
\subsection{Sniffing the test device}
The described sniffing and analysis procedure will be used in the next section to
record USB traffic from the scanner while it is transmitting an image. The transmitted
image should then be reconstructed from this recording and compared to the image
the vendor software saves to disk.
...
It is important to use a kernel capable of recording the full payload (\cref{ssec:kernelpatch})
and also to disable extra protocols in Wireshark (\cref{nonusb}).
...
incoming from endpoint 1 (\cref{fig:large_bulkxfers}). It seems likely these contain the image data.
...
seem almost random (\cref{fig:evenoddoffset}).
This suggests the scanner delivers uncompressed 16 bit per pixel little-endian data:
...
The transferred bytes form three distinct images (\cref{fig:extract_calibprevmain}): The first section
...
while the calibration scan is analyzed in \cref{ssec:calibration}.
...
\begin{figure}[H]
  ...
  \label{fig:extract_calibprevmain}
\end{figure}

\subsubsection{Line width and offsets}
...
Only the high bytes were kept to transform the 16-bit pixel values to 8-bit grayscale (\cref{capture_highlowbytes}),
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[offset too low: 0]{{\includegraphics[height=6cm]{images/jpg/extract_image_3.jpg} }}
  \hspace{3em}
  \subfloat[offset too high: 600]{{\includegraphics[height=6cm]{images/jpg/extract_image_4.jpg} }}
...
When looking at the image of \cref{fig:extract_step1} in detail,
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[offset 840]{{\includegraphics[height=3.5cm]{images/jpg/extract_image_6a.jpg} }} \quad
  \subfloat[offset 841]{{\includegraphics[height=3.5cm]{images/jpg/extract_image_6b.jpg} }}
  
  \subfloat[offset 842]{{\includegraphics[height=3.5cm]{images/jpg/extract_image_6c.jpg} }} \quad
  \subfloat[offset 843]{{\includegraphics[height=3.5cm]{images/jpg/extract_image_6d.jpg} }}
\end{figure}
...
The infrared channel will be used in \cref{sec:imgproc} to remove dust at the other color channels.

\subsubsection{Channel alignment, combination, gamma correction}
\label{sssec:channelcombine}

In a line image sensor, the detectors for different channels are stacked vertically, this
means the image for each channel will be a few rows out of alignment with the other channels.
To realign, the channel that is the furthest "up" (i.e. the channel for which all other channels only need
upwards shifting to align) is determined. Then, for each of the other channels the number
of lines it needs to be moved up to be aligned is determined. This is done visually in GIMP by
...
(because of interleaving) and added to the channel line offsets found in \cref{ssec:deinterleave}.
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[extracted from capture]{{\includegraphics[width=0.6\textwidth]{images/jpg/extract_image_7a.jpg} }}
  
  \subfloat[saved by CyberView X5]{{\includegraphics[width=0.6\textwidth]{images/jpg/extract_image_7b.jpg} }}
\end{figure}
...
the help of the calibration scan (\cref{ssec:calibration}).
CyberView also removes dust using the infrared channel (called Digital Image Correction and Enhancement).
...
This image can be used to "subtract" (actually a division operation) the unwanted
brightness differences from the actual scan.
The calibration image is transferred before the other scans (\cref{ssec:extract_results}).
...
Determining the line width is harder because we do not have a rough guess like the
vendor software image width and there is no image details on the calibration image. It helps that there
is a pattern (darker pixel) at the start of each line. Thus, we can extract the image
with any line size ($w_{chosen}$) and count the difference in rows and columns between two successive
line starts. We then get the line width by $w_{correct} = (y_b - y_a) \cdot w_{chosen} + x_b - x_a$.
where $y_a, x_a$ are the row and column of the point of any line start pattern
and $y_b, x_b$ row and column the line start pattern following the first one.
...
In this case, the line width was found to be $(115 - 44) \cdot 300 + 166 - 102 = 21364$ pixels.
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[red]{{\includegraphics[height=1cm]{images/jpg/extract_calib_3r.jpg} }}
  \subfloat[green]{{\includegraphics[height=1cm]{images/jpg/extract_calib_3g.jpg} }}
  
  \subfloat[blue]{{\includegraphics[height=1cm]{images/jpg/extract_calib_3b.jpg} }}
  \subfloat[infrared]{{\includegraphics[height=1cm]{images/jpg/extract_calib_3i.jpg} }}
\end{figure}

Applying the calibration image to the actual scan is shown in \cref{ssec:imgproc_calib}.

\section{Controlling the device}

A Python module for writing USB drivers was used to create a simple scanning program
{\it scanInteractive.py} which performs a scan with certain parameters, does basic image processing
(see \cref{sec:extract}) and displays the image building up row by row
...
vendor and product ID (these are for the test device, can be found via {\it lsusb}):
...
{\it bmRequestType, bRequest, wValue, wIndex, wLength} are described in \cref{table:ctrltransfer}.
The timeout specifies the number of milliseconds after which the transfer is canceled if
still not completed.
...
in both directions: bmRequestType 0x40 outgoing / 0xc0 incoming (\cref{table:ctrltransfer}).
...
In addition to the parameters, control transfers have a data payload that is {\it wLength} bytes long (\cref{table:ctrltransfer}).
...
Every transaction begins with the same header that consists of the 11 control
transfers shown in in \cref{table:transheader}.

\begin{table}[H]
  \caption{Header structure for all three transaction types.}
  \centering
  \begin{tabular}{l|l|l}
    Nr. & Parameters (\cref{table:paramcombos}) & Data payload \\ \hline
    1. & e) & 0xff \\
    2. & e) & 0xaa \\
    3. & e) & 0x55 \\
    4. & e) & 0x00 \\
...
This is done using a control transfer with parameter combination b) in \cref{table:paramcombos}.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l | X}
    ...
    {\tt 0x03} & Read another response byte (\cref{table:devready_2}) \\
  \end{tabularx}
\end{table}
...
First, a control transfer (combination a) in \cref{table:paramcombos}) is
...
The structure of the basic transaction is shown in \cref{table:basictransaction}.

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{X | X | X | X}
    \caption{Structure of the basic transaction} \\
    \label{table:basictransaction}
    
    Stage & No. of transfers / bytes & Transfer parameters & Data payload \\ \hline
    
    Header & 11 & \cref{table:transheader} & \cref{table:transheader} \\
    Main parameters & 6 & c) in \cref{table:paramcombos} & \cref{appendix_basic} \\
    Device response & 2 & b) in \cref{table:paramcombos} & \cref{ssec:devready} \\
  \end{tabularx}
\end{table}
...
The structure of the extra parameter transaction is shown in \cref{table:extratransaction}.

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{X|X|X|X}
    \caption{Structure of the extra parameter transaction} \\
    \label{table:extratransaction}
    
    Stage & No. of transfers / bytes & Transfer parameters & Data payload \\ \hline
    
    Header & 11 & \cref{table:transheader} & \cref{table:transheader} \\
    Main parameters & 6 & c) in \cref{table:paramcombos} & \cref{appendix_extra} \\
    Device response & 1 & b) in \cref{table:paramcombos} & \cref{ssec:devready}, {\tt 0x00} expected \\
    Extra parameters & varies (\cref{appendix_extra}) & c) in \cref{table:paramcombos} & \cref{appendix_extra} \\
    Device response & 2 & b) in \cref{table:paramcombos} & \cref{ssec:devready} \\
  \end{tabularx}
\end{table}
...
The structure of the status transaction is shown in \cref{table:statustransaction}.

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{X|X|X|X}
    \caption{Structure of the read transaction} \\
    \label{table:statustransaction}
    
    Stage & No. of transfers / bytes & Transfer parameters & Data payload \\ \hline
    
    Header & 11 & \cref{table:transheader} & \cref{table:transheader} \\
    Main parameters & 6 & c) in \cref{table:paramcombos} & \cref{appendix_status} \\
    Device response & 1 & b) in \cref{table:paramcombos} & \cref{ssec:devready}, {\tt 0x01} expected \\
    Bulk read & \cref{appendix_status} & \cref{ssec:bulkread} & \cref{ssec:bulkread} \\
    Device response & 2 & b) in \cref{table:paramcombos} & \cref{ssec:devready} \\
  \end{tabularx}
\end{table}
...
The structure of the image transaction is shown in \cref{table:imgtransaction}.

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{X|X|X|X}
    \caption{Structure of the image transaction} \\
    \label{table:imgtransaction}
  
    Stage & No. of transfers / bytes & Transfer parameters & Data payload \\ \hline
    
    Header & 11 & \cref{table:transheader} & \cref{table:transheader} \\
    Main parameters & 6 & c) in \cref{table:paramcombos} & \cref{ssec:image_param} \\
    Device response & 1 & b) in \cref{table:paramcombos} & \cref{ssec:devready} \\
    Bulk reads & \cref{ssec:image_bulk} & \cref{ssec:bulkread} & \cref{ssec:bulkread} \\
    Device response & 2 & b) in \cref{table:paramcombos} & \cref{ssec:devready} \\
  \end{tabularx}
\end{table}
...
"transfer 14 lines" after the scan has started), illustrated by the following
comment in the SANE driver for this device {\cite[\tt pieusb\_scancmd.c, sanei\_pieusb\_cmd\_start\_scan()]{sane_code}}:

\blockquote[]
{\it This is a SCSI READ command (code 0x08).
...
bulk reads (\cref{ssec:bulkread}).

For this, the amount of bytes per line of the image $c_l$ needs to be known
(\cref{table:imageparam_query}). This is multiplied by the number of lines $l$ to be transferred
...
The bytes are transferred in $n$ 65520-byte reads followed by one $m$-byte
read (if m is nonzero), where
...
The size and format of the calibration image do not depend on the scanner settings.
...
The lines follow an RGBI channel interleaving (\cref{sec:extract}).

In the actual scan (\cref{ssec:actualscan}), at first 4 lines get transferred, then 172, then the last 4.
...
The number of lines $l$ gets divided into $p$ 255-line chunks plus a possible rest $q$, where

\begin{center}
\begin{tabular}{l}
$p = \lfloor \frac{l}{255} \rfloor $ \\
$q = l \mod 255$ .\\
\end{tabular}
\end{center}

The image is transferred in $p$ 255-line image transactions followed by one $q$-line
image transaction (if $q$ is nonzero). Each image transaction needs
...
image transaction. To ensure device readiness, a basic query (\cref{ssec:basic_trans})
is used. The device response (\cref{ssec:devready}) in this query returns "busy" if the device
...
until the device responds that it is not busy.
...
and the image transaction should be repeated from the start (\cref{ssec:rcv_mainimage}).
...
\cref{time_diagram} illustrates the rate at which packets are transmitted
...
(\cref{ssec:read_trans}) is performed.

\item When the user presses "start scan", all scan parameters being transferred
in rapid succession cause a quick increase in packet count.
...
in \cref{appendix_actualscan}.

\section{Scanner Access Now Easy}
\label{sec:sane}
...
\begin{figure}[H]
  ...
  \centering
  \resizebox{\textwidth}{!}{\input{images/tikz/sane_hierarchy.tex}}
  \label{sane_hierarchy}
\end{figure}

\subsection{Backend / frontend interface}
\label{ssec:sanestd}
...
title (human-readable name), description, type (bool / string ...), unit (pixels / millimeters ...)
and constraints (e.g., a range of possible coordinate values in a dimension,
...
descriptors' meanings are specified in the standard (\cref{table:optiondescriptors}).

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{l | X}
    \caption{Special option descriptors specified in the SANE standard \cite[4.5]{sane_standard}.} \\
    \label{table:optiondescriptors}
...
Option values are queried/set using the {\it sane\_control\_option} procedure
specifying the option index and a generic pointer to load / store the value.
This also reports back on how well the request could be met (e.g., a resolution could
...
scanning, and retrieving data (\cref{table:saneoperations}).
Typical code flow using these operations during normal scanner usage is illustrated
in \cref{sane_codeflow}.

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{l | X}
    \caption{Operations defined in the SANE standard (not exhaustive) \cite[4.3]{sane_standard}} \\
    \label{table:saneoperations}
...
\begin{figure}[H]
  ...
  \centering
  \scalebox{1}{\input{images/tikz/sane_codeflow.tex}}
  \label{sane_codeflow}
\end{figure}
...
\cref{sec:sane} ({\it dll.conf}).

\subsubsection{Fixing I/O errors}

For the test device, scanning unfortunately did not work out of the box. When testing operation
...
parameters, so it seems the offending commands were superfluous for the test device.
...
The raw pixel data coming from the scanner does not yet make for a usable image.
Many processing steps need to be done before the scan looks presentable. Some are
specific to this model of scanner, these are described in \cref{sec:extract}. Generic
image processing operations dealing with contrast, colors, dust removal and cropping are described in this section.
...
Retrieving the calibration image is described in \cref{ssec:rcv_calibimage}, extracting it
from a capture file in \cref{ssec:calibration}. For the test device the calibration image is always
5341 pixels wide and 45 pixels high (for each of the 4 channels red, green, blue, infrared,
...
Retrieving this bitmask is specified in \cref{appendix_status}.
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[before calibration]{{\includegraphics[width=0.5\textwidth]{images/jpg/imgproc_calib_1.jpg} }}
  \subfloat[calibration image]{{\includegraphics[width=0.5\textwidth]{images/jpg/imgproc_calib_2.jpg} }}
  
  \subfloat[division $a / b$]{{\includegraphics[width=0.5\textwidth]{images/jpg/imgproc_calib_3.jpg} }}
\end{figure}
...
To increase the brightness by $k$, a function $b'_x = b_x + k$ is used.

\subsubsection{Contrast}

Changing contrast means compressing the brightness function so that its climb
is steeper at the middle.
To increase the contrast by a factor $k > 0$, a function $b'_x = (b_x - 0.5) * k + 0.5$ is used.
...
To apply gamma correction for a $k > 0$, a function $b'_x = b_x^k$ is used.
If $k < 1$, the function will be bent to the right so the majority of the climb
happens in darker areas.
If $k > 1$, the function will be bent to the left so the majority of the climb
happens in lighter areas.
...
It can be represented by $b'_x = \frac{b_x - k_1}{k_2 - k_1}$ where $k_1$ is the lower
...
level operations (\cref{fig:colorcorrect}).
...
\begin{figure}[H]
  ...
  \subfloat[raw scan]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_levels_1.jpg}} \ 
  \subfloat[inverted]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_levels_2.jpg}}

  \subfloat[histogram of (b)]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_levels_2h.jpg}} \ 
  \subfloat[levels stretched to actual value range]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_levels_3.jpg}}
  
  \label{fig:colorcorrect}
\end{figure}
...
To deal with this, a technique often called Digital Image Correction and Enhancement \cite{inpainting_dice} was developed.
It involves scanning the film in the infrared spectrum in addition to the red, green, and blue bands.
...
\begin{figure}[H]
  ...
  \subfloat[RGB image]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_inpaint_1.jpg}} \ 
  \subfloat[infrared channel]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_inpaint_2.jpg}}
\end{figure}
...
Finding scratches is not so easy as they do not have a strong brightness difference
...
It may also be desirable to use dilation to grow the area around affected pixels somewhat
so the pixels just outside the affected range, i.e. those used as a template
to paint over the affected pixels, are certainly unaffected and not harming the inpainting process.
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[1 iteration]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_inpaint_4a.jpg}} \ 
  \subfloat[10 iterations]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_inpaint_4b.jpg}}
  
  \subfloat[100 iterations]{\includegraphics[width=0.49\textwidth]{images/jpg/imgproc_inpaint_4c.jpg}}
\end{figure}
...
\cref{fig:cropalgo_result} shows this algorithm applied to a real scan: Detected lines
are in red and orange, intersection points in green, and the blue rectangle is the resulting crop area.

\begin{figure}[H]
  \caption{Applying crop algorithm to image}
  \centering
  \includegraphics[width=\textwidth]{images/jpg/imgproc_autocrop_2.jpg}
  \label{fig:cropalgo_result}
\end{figure}
...
focus on the types and structure of control transfers (\cref{sec:usb}). After that, various methods of
sniffing USB traffic were compared (\cref{ssec:snifferscompared}). Software sniffing is done using the usbmon
Linux kernel module. To allow access to all of the payload of large packets / control transfers,
a simple patch had to be applied to this kernel module (\cref{ssec:kernelpatch}). Wireshark / tshark was used
to decode raw packets and extract control transfer parameters (\cref{ssec:wireshark}). The vendor-supplied
software was run on Windows inside VirtualBox (using the USB passthrough feature)
while letting Wireshark sniff on the Linux host (\cref{ssec:virtualbox}).

As a partial goal, the image was reconstructed from the captured packet stream (\cref{sssec:channelcombine}).
Then, the packet sequence was studied for patterns. All traffic could be modeled
using four types of transaction (\cref{ssec:transactionpatterns}). By trial and error the meaning of some of the bytes
in the transaction parameters was uncovered (\cref{appendix_meaning}). Keeping the importance of proper timing
...
to make it work with the scanner, most features are supported (\cref{sec:sane}).
...
from the scanner becomes usable (\cref{sec:imgproc}). The basic steps include assembling the image
...
was presented (\cref{ssec:imgproc_dice}). A method for automatically cropping
away the unexposed area of the film was demonstrated (\cref{ssec:imgproc_crop}).

There are some aspects of scanner behavior that make using the device
more complex and were not covered in this work:

\begin{itemize}
\item The lines from the different color channels can be interleaved in different patterns / sequences,
the "RGBIRGBI..." order was assumed in this work.
The pattern by which the device interleaves the channels is chosen based on the "color format" parameter
(\cref{table:param15}) and possibly the resolution and color mode.
...
\paragraph{Scan frame}

The parameter string "0a0000000e00" is used to set the boundaries of the scan
...
\paragraph{Scan resolution} The parameter string "150000001000" is used to set many different scan parameters
...
\paragraph{Gain / offset} The parameter string "dc0000001d00" is used to set some parameters such as exposure time
...
studied in detail. Bytes at offset 0, 3, 4 can be set. Bytes 3 and 4 specify the number of bytes to
be read from the bulk endpoint: $256 * b_3 + b_4$.

\paragraph{Image dimensions}

This parameter string "0f0000001200" is used to receive information about the image produced by the scanner.
...
\begin{tabularx}{\textwidth}{p{0.2cm}|p{2.3cm}|p{2cm}|X}
  ...
  \\ \caption*{Transaction types: B - Basic, E - Extra parameter, S - Status, I - Image} \\
  \endlastfoot
  S & {\tt dd0000001200} &  & Idle status query (\cref{appendix_status}) \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}) \\
  ...
  S & {\tt 080000008000} &  & \\
  E & {\tt 0a0000000e00} & \tt 12000a0080 0000000000 b829e71a & Set scan area (\cref{table:param0a}) \\
  E & {\tt 0a0000000600} & \tt 1700020001 00 & \\
  S & {\tt 030000000e00} &  & \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}) \\
  S & {\tt d70000006700} &  & Read gain / offset values (\cref{appendix_status}) \\
  E & {\tt dc0000001d00} & \tt 0000000000 0000000000 0000000000 07001a790b 00fe0f0000 00000000
                         & Set exposure settings (\cref{table:paramdc}) \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}) \\
  E & {\tt 150000001000} & \tt 000f2c0180 0404000100 0000008010 00 & Set gain / offset values (\cref{table:paramdc}) \\
  B & {\tt 1b0000000100} &  & Start scan (\cref{ssec:basic_trans}) \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}), 3rd attempt is successful \\ 
  S & {\tt d70000006700} &  & Read gain / offset values (\cref{appendix_status}) \\
  E & {\tt dc0000001d00} & \tt 7b1e2f168c 1017141000 0000212121 070000790b 14000f0000 00000000
                         & Set exposure settings (\cref{table:paramdc}) \\
  I & {\tt 080000000400} &  & Transfer 4 lines / 42728 bytes of the calibration image (\cref{ssec:rcv_calibimage}) \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}), 2nd attempt is successful \\
  I & {\tt 08000000ac00} &  & Transfer 172 lines / 1837304 bytes of the calibration image (\cref{ssec:rcv_calibimage}) \\
  S & {\tt d70000006700} &  & Read gain / offset values (\cref{appendix_status}) \\
  E & {\tt dc0000001d00} & \tt 7b1e2f168c 1017141000 0000212121 070000790b 14000f0000 00000000
                         & Set exposure settings (\cref{table:paramdc}) \\
  I & {\tt 080000000400} &  & Transfer 4 lines / 42728 bytes of the calibration image (\cref{ssec:rcv_calibimage}) \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}), 2nd attempt is successful \\
  S & {\tt 18000014dc00} &  & Read row mask (\cref{appendix_status}) \\
  S & {\tt 0f0000001200} &  & Get size of the main image (\cref{table:imageparam_query}).
                              Returned status is {\tt bc011f01bc01...}. This means the
                              image has 287 * 3 (color channels) = 861 lines of 444 bytes each. \\
  B & {\tt 000000000000} &  & Readiness check (\cref{ssec:basic_trans}), 3rd attempt is successful \\
  I & {\tt 08000000d800} &  & Transfer 216 lines / 95904 bytes of the main image (\cref{ssec:rcv_mainimage}).
                              Repeated 3 times.
