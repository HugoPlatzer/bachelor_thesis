...
\begin{figure}[H]
  ...
  \centering
  \subfloat[extracted from capture]{{\includegraphics[width=0.6\textwidth]{images/extract_7a.jpg} }}
  
  \subfloat[saved by CyberView X5]{{\includegraphics[width=0.6\textwidth]{images/extract_7b.jpg} }}
\end{figure}
...
Details about the transaction parameters can be found in \ref{appendix_basic}.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3cm} | p{1.5cm} | p{3cm} | p{3cm}}
    ...
    Main parameters & 6 & c) in \autoref{table:paramcombos} & see \ref{appendix_basic} \\
...
Details about the transaction parameters can be found in \ref{appendix_extra}.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3cm} | p{1.5cm} | p{3cm} | p{3cm}}
    ...
    Header & 11 & see \autoref{table:transheader} & see \autoref{table:transheader} \\
    Main parameters & 6 & c) in \autoref{table:paramcombos} & see \ref{appendix_extra} \\
    Device response & 1 & b) in \autoref{table:paramcombos} & see \ref{ssec:devready}, {\tt 0x00} expected \\
    Extra parameters & varies, see \ref{appendix_extra} & c) in \autoref{table:paramcombos} & see \ref{appendix_extra} \\
    Device response & 2 & b) in \autoref{table:paramcombos} & see \ref{ssec:devready} \\
  \end{tabular}
\end{table}
...
Details about the transaction parameters can be found in \ref{appendix_status}.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3cm} | p{1.5cm} | p{3cm} | p{3cm}}
    ...
    Header & 11 & see \autoref{table:transheader} & see \autoref{table:transheader} \\
    Main parameters & 6 & c) in \autoref{table:paramcombos} & see \ref{appendix_status} \\
    Device response & 1 & b) in \autoref{table:paramcombos} & see \ref{ssec:devready}, {\tt 0x01} expected \\
    Bulk read & see \ref{appendix_status} & see \ref{ssec:bulkread} & see \ref{ssec:bulkread} \\
    Device response & 2 & b) in \autoref{table:paramcombos} & see \ref{ssec:devready} \\
  \end{tabular}
\end{table}
...
Retrieving this bitmask is specified in \ref{appendix_status}.
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[before calibration]{{\includegraphics[width=0.5\textwidth]{images/calib_a.jpg} }}
  \subfloat[calibration image]{{\includegraphics[width=0.5\textwidth]{images/calib_b.jpg} }}
  
  \subfloat[division $a / b$]{{\includegraphics[width=0.5\textwidth]{images/calib_c.jpg} }}
\end{figure}
...
source increases. To make a scan of negative film look normal it needs to be finverted:
$f(b) = 1 - b$.
...
\begin{figure}[H]
  ...
  \subfloat[raw scan]{\includegraphics[width=0.49\textwidth]{images/imgop_color1.jpg}} \ 
  \subfloat[inverted]{\includegraphics[width=0.49\textwidth]{images/imgop_color2.jpg}}

  \subfloat[histogram of (b)]{\includegraphics[width=0.49\textwidth]{images/imgop_color_h.jpg}} \ 
  \subfloat[levels stretched to actual value range]{\includegraphics[width=0.49\textwidth]{images/imgop_color3.jpg}}
\end{figure}
...
\begin{figure}[H]
  ...
  \subfloat[RGB image]{\includegraphics[width=0.49\textwidth]{images/inpaint_1.jpg}} \ 
  \subfloat[infrared channel]{\includegraphics[width=0.49\textwidth]{images/inpaint_2.jpg}}
\end{figure}
...
\begin{figure}[H]
  ...
  \centering
  \subfloat[1 iteration]{\includegraphics[width=0.49\textwidth]{images/inpaint_4a.jpg}} \ 
  \subfloat[10 iterations]{\includegraphics[width=0.49\textwidth]{images/inpaint_4b.jpg}}
  
  \subfloat[100 iterations]{\includegraphics[width=0.49\textwidth]{images/inpaint_4c.jpg}}
\end{figure}
...
focus on the types and structure of control transfers \ref{sec:usb}. After that, various methods of
sniffing USB traffic were compared \ref{ssec:snifferscompared}. Software sniffing is done using the usbmon
Linux kernel module. To allow access to all of the payload of large packets / control transfers,
a simple patch had to be applied to this kernel module \ref{ssec:kernelpatch}. Wireshark / tshark was used
to decode raw packets and extract control transfer parameters \ref{ssec:wireshark}. The vendor-supplied
software was run on Windows inside VirtualBox (using the USB passthrough feature)
while letting Wireshark sniff on the Linux host \ref{ssec:virtualbox}.

As a partial goal, the image was reconstructed from the captured packet stream \ref{sssec:channelcombine}.
Then, the packet sequence was studied for patterns. All traffic could me modeled
using four types of transaction \ref{ssec:transactionpatterns}. By trial and error the meaning of some of the bytes
in the transaction parameters was uncovered \ref{appendix_meaning}. Keeping the importance of proper timing
...
to make it work with the scanner, most features are supported \ref{sec:sane}.
...
from the scanner becomes usable \ref{sec:imgproc}. The basic steps include assembling the image
...
was presented \ref{ssec:imgproc_dice}. A method for automatically cropping
away the unexposed area of the film was demonstrated \ref{ssec:imgproc_crop}.
...
\begin{longtable}{p{0.4cm} | p{2.3cm} | p{2cm} | p{5cm}}
  ...
  \\ \caption*{Transaction types: B - Basic, E - Extra parameter, S - Status, I - Image} \\
  \endlastfoot
  S & {\tt dd0000001200} &  & Idle status query \ref{appendix_status} \\
  ...
  S & {\tt 030000000e00} &  & \\
  B & {\tt 000000000000} &  & Readiness check \ref{ssec:basic_trans} \\
  S & {\tt d70000006700} &  & Read gain / offset values \ref{appendix_status} \\
  ...
  I & {\tt 080000000400} &  & Transfer 4 lines / 42728 bytes of the calibration image \ref{ssec:rcv_calibimage} \\
  B & {\tt 000000000000} &  & Readiness check \ref{ssec:basic_trans}, repeated twice until successful \\
  S & {\tt 18000014dc00} &  & Read row mask \ref{appendix_status} \\
...
\begin{thebibliography}{}
  ...
  \bibitem{inpainting_diffusion} Fast Digital Image Inpainting: Manuel M. Oliveira, Brian Bowen, Richard McKenna, Yu-Sung Chang. Proceedings of the International Conference on Visualization, Imaging and Image Processing (VIIP 2001).
