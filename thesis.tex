...
Many backend procedures return a value of type {\it SANE\_Status} to indicate
success or various kinds of failure.

The standard defines various primitive data types ({\it SANE\_Byte,
SANE\_Word, SANE\_Char, SANE\_String}) which have certain guaranteed qualities
like minimum number of bytes, endianness etc. to migitate differences in
C compilers and hardware. Then there is the {\it SANE\_Handle} data type,
which is used for accessing a scanner using the backend.
