...
\begin{table}[H]
  ...
  \begin{tabular}{l | l | p{2cm} | p{2cm}}
    ...
    SET\_CONFIGURATION & 0000000 & Configuration value & Zero \\
  \end{tabular}
...
The host will learn about a new device being plugged in by being notified
...
new device from its default address zero to an unused one. This is called
enumeration.

After it has been assigned an address, the host queries various descriptors
(especially the Device and Configuration Descriptor) identifying the capabilities
of the device.

If the host supports the device (i.e. has the right drivers) it will choose one
of the device's configuration profiles. This gets the device into configured
state and allows it to communicate with the suitable driver using a device-specific
protocol. \cite[p. 87ff.]{uc}
