...
usbmon is a module for the Linux kernel that allows access to
USB request blocks (URBs) as they are being processed. \cite{usbmon}
...
Its configurable interface allows the dissection of raw captured packets
into more meaningful fields.
For sniffing USB traffic, it requires the usbmon kernel module. \cite{wireshark}

\subsection{Example}

A USB keyboard is attatched, then the following keys are being pressed and
then released in sequence: a, b, a.
...
\begin{figure}[H]
  \caption{Interface descriptor of USB keyboard}
  \centering
  \includegraphics[width=\textwidth]{images/keyboard_sniff2.png}
\end{figure}

\begin{figure}[H]
  \caption{HID reports of USB keyboard}
  \centering
