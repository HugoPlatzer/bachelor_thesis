...
In addition to the parameters, control transfers have a data payload that is {\it wLength} bytes long \autoref{table:ctrltransfer}. The actual commands regarding what the device should do are encoded in this data payload.
...
are four types of transaction: The most basic does not do anything besides sending parameters and querying the response. One kind of transaction transmits additional parameter bytes, another reads some more bytes of device status and the fourth kind is used
for retrieving image data.
...
\begin{table}[H]
  \caption{Arguments of the basic transaction}
  \centering
  \begin{tabular}{c | c | p{5cm}}
    Argument & Byte count & Description \\ \hline
    
    Main parameters & 6 & add some description \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{c | c | c | p{4cm}}
    Stage & Packet count & Transfer parameters & Data payload \\ \hline
    
    Header & 11 & see \autoref{table:transheader} & see \autoref{table:transheader} \\
    Main parameters & 6 & c) in \autoref{table:paramcombos} & Main parameters \\
    Device response & 2 & b) in \autoref{table:paramcombos} & See the extra text \\
  \end{tabular}
\end{table}
...
\subsection{Device readiness response}

\subsection{Timing}

\subsection{Performing a scan}
