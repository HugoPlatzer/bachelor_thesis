...
side of the host. They are composed of transactions, which usually consist of three
packets: 
...
Every device must have endpoint 0 for control transfers in both directions.
They start with a Setup transaction specifying the target device and endpoint plus
an extra 8 bytes of data. These 8 bytes tell the type and parameters of the control transfer.
Depending on the kind of control transfer, there may be data
transactions for additional information, but this is not required.
The transfer is completed with a Status transaction to report back about the success of the whole transfer.
